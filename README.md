...
Build: docker build -t whisperx-web .
Run: docker run --gpus all -p 7860:7860 whisperx-web (or --rm -p 7860:7860 for CPU-only)
...
Open web UI.
Upload files.
Select model, format, diarization (enter HF token).
Click "Transcribe".
...
User auth/sessions for multi-user.
